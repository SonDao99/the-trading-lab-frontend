...
const generateData = (count: number): BalanceData[] => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        data.push({ tradeNumber: i + 1, balance: parseFloat(balance.toFixed(2)) })
    }
    return data
}
...
export default function BacktestChart() {
    ...
    const drawChart = () => {
        ...
        ctx.lineTo(canvas.width / scale - margin.right, canvas.height / scale - margin.bottom)
        ctx.stroke()
            ...
            if (
                ...
                if (yScaleRef.current && zoomedXScaleRef.current) {
                    ...
                    if (xValue >= 0 && xValue < data.length) {
                        ...
                        hoverCtx.strokeStyle = yValue >= startingBalance ? '#4ade80' : '#ef4444'
    ...
    return (
        <div ref={containerRef} className="canvas-container min-h-[90vh] w-full bg-gray-900">
